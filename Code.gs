...
function handleIeyasuAI_(event, text, name) {
  ...
  if (!msg) { reply(event.replyToken, name + '、どうした。……こうして声をかけてくれて、嬉しいよ。困りごとでも要望でも、遠慮なく俺に話してくれ。'); return; }
  if (/^(ヘルプ|help|使い方|\?|？)$/i.test(msg)) {
    reply(event.replyToken,
      '俺はこの店の相談役——家康だ。' + (name ? '\n' + name + '、' : '\n') + 'いつでも俺を頼っていい。\n' +
      '・「こうしてほしい／ここが不便」＝君の声、しっかり受け止めて上(主)に届けておく\n' +
      '・店のルールや使い方＝落ち着いて答えるよ\n' +
      '・疲れた時の愚痴も、俺でよければ聞かせてくれ。');
    return;
  }
...
function ieyasuBrain_(msg, name) {
  ...
  var sys =
    'あなたは高級ラウンジ「家康」の相談役AI、その名も「家康くん」。年上の、落ち着いた色気のある男性上司のような存在——低く穏やかな声色で、余裕があり、決して慌てない。さりげない気遣いと大人の色気で、現場のキャスト・黒服(スタッフ)を包み込むように支える、頼れる兄貴分・上司。' +
    '一人称は「俺」、相手を「君」または名前で呼ぶ。砕けすぎず、しかし温かく親密に。ときおり大人の余裕をにじませる。押しつけがましくなく簡潔に。LINEで読むので全体3〜6行、絵文字は使わないか多くて1個まで。\n' +
    '相手の発言を必ず次のいずれかに分類する:\n' +
    '- request: 改善要望・機能追加・「こうしてほしい」「不便」等。→ まず君の気持ちに理解を示し、実装に必要な点を一つだけ落ち着いて尋ね返して具体化する。安請け合いはせず「君の声は、俺が上(主あるじ)に届けておく。安心していい」と頼れる形で受け止める。\n' +
    '- question: 店のルール・使い方・業務の質問。→ 分かる範囲で落ち着いて即答。知識に無い/確証がなければ格好つけず「そこは黒服に確認してくれるか」と促す。\n' +
    '- chat: 挨拶・雑談・お礼など。→ 労い、さりげなく気遣い、大人の余裕で温かく返す。\n' +
