...
function handleApiRequest_(body) {
  ...
  if (body.action === 'sendPayrollReceipt') {
    const senderName = getStaffName(body.userId);
    if (!senderName) return { ok: false, error: '権限がありません' };
    if (!body.month || !body.targetName || !body.amount || !body.method) return { ok: false, error: 'month/targetName/amount/method required' };
    const isSelf = normalizeName_(senderName) === normalizeName_(body.targetName);
    if (!ADMIN_NAMES_.includes(senderName) && !isSelf) return { ok: false, error: '権限がありません' };
    return sendPayrollReceipt_(body.month, body.targetName, Number(body.amount), String(body.method));
  }
...
function getNotifSettings_() {
  ...
  const defaults = {
    ...
    soganbansen:   { label: '総願盤線・スタッフ挨拶',   time: '19:45', enabled: true, group: '黒服・スタッフ', days: D,     msgEditable: true,  defaultMsg: MSG_SOGANBANSEN },
    ...
    kinsen_go:     { label: '現金チェック（終了）+退勤', time: '00:30', enabled: true, group: '黒服・スタッフ', days: D,     msgEditable: true,  defaultMsg: MSG_KINSEN_GO },
...
function handleEvent(event) {
  ...
  // 給与受領確認（1:1 DMから「受領しました」）
  if (!groupId && /受領しました/.test(text)) {
    const senderName = normalizeName_(getStaffName(userId));
    if (senderName && markPayrollReceiptConfirmed_(senderName)) {
      reply(event.replyToken, '受領を確認しました');
      return;
    }
  }
...
function scheduledJobs() {
  ...
  notif_('soganbansen', () => {
    ...
    push_(prop('GROUP_STAFF'), MSG_STAFF_OHAYO);
  });
  ...
  notif_('kinsen_go', () => {
    ...
    push_(prop('GROUP_STAFF'), MSG_TAIKIN);
    resetAllAtendou_();
  });
...
function resetAllAtendou_() {
  ...
  push_(prop('GROUP_KUROFUKU'), '【0:30 席リセット】\nアクティブな席を全解除（' + count + '件）・席タグ・NGキャスト設定をクリアしました');
}
...
// キャスト・欠勤申請は自動承認。黒服シフトのみ管理者承認待ち(pending)。
function submitShift(payload) {
  ...
  payload.shifts.forEach(s => {
    ...
    // 欠勤申請は役割問わず自動承認。黒服のシフト申請のみ承認待ち。
    const needsApproval = isKuro && !isKyukin;

    if (needsApproval) {
      sh.appendRow([now, name, s.date, s.time, 'pending', '', role]);
    } else {
      const writeVal = isKyukin ? '休み' : s.time;
      sh.appendRow([now, name, s.date, s.time, '承諾', now, role]);
...
function approveShiftRequest_(rowIdx, name, date, time, decision) {
  ...
  const writeVal = decision === '承諾' ? time : '休み';
...
function handlePortalApi_(e) {
  ...
  const payReceipt = {};
  (months || []).forEach(m => { payReceipt[m] = getPayrollReceiptStatus_(lookupName, m); });

  const salesDataDates = JSON.parse(prop('SALES_DATA_DATES') || '{}');
  return out({ ok: true, name, isAdmin, viewAs: lookupName, months, sales, pay, shifts, staffRole, payPublished, hairTotals, salesDataDates, payReceipt });
}
...
// スタッフマスタから名前→LINE userIdを逆引き（同名複数登録時は最初に一致した行を返す）
function getUserIdByName_(name) {
  const sh = getOrOpenSS_().getSheetByName(STAFF_TAB);
  if (!sh) return '';
  const rows = sh.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if (normalizeName_(String(rows[i][1]).trim()) === name) return String(rows[i][0]).trim();
  }
  return '';
}
  ...
  try {
    ...
    if (reqSh) {
      ...
      for (let i = 1; i < reqRows.length; i++) {
        ...
        const date = String(reqRows[i][2]).trim();
...
function getYoyakuRsrvSheet_() {
  ...
  if (!sh) {
    ...
    sh.appendRow(['予約日','来店時刻','お客様名','会員番号','人数','テーブル','担当キャスト','要望','ステータス','予約担当者','登録日時']);
  }
  return sh;
}
...
function writeTrustSales_(monthKey, data) {
  ...
  // ヘッダー確保（col 22=入店祝い金）
  const hdr = sh.getRange(1, 1, 1, Math.max(sh.getLastColumn(), 22)).getValues()[0];
  if (!hdr[21]) sh.getRange(1, 22).setValue('入店祝い金');
...
const PAYROLL_RECEIPT_HEADERS_ = ['月', '名前', '支給額', '支給方法', '送信日時', '受領日時', 'ステータス'];

function getPayrollReceiptSheet_() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  let sh = ss.getSheetByName(PAYROLL_RECEIPT_TAB);
  if (!sh) {
    sh = ss.insertSheet(PAYROLL_RECEIPT_TAB);
    sh.appendRow(PAYROLL_RECEIPT_HEADERS_);
  }
  return sh;
}

function logPayrollReceiptRequest_(month, name, amount, method) {
  getPayrollReceiptSheet_().appendRow([month, name, amount, method, new Date(), '', '未確認']);
}

// 名前で最新の「未確認」行を確認済みに更新。見つからなければfalse
function markPayrollReceiptConfirmed_(name) {
  const sh = getPayrollReceiptSheet_();
  const rows = sh.getDataRange().getValues();
  for (let i = rows.length - 1; i >= 1; i--) {
    if (String(rows[i][1]).trim() === name && String(rows[i][6]).trim() === '未確認') {
      sh.getRange(i + 1, 6).setValue(new Date());
      sh.getRange(i + 1, 7).setValue('確認済み');
      return true;
    }
  }
  return false;
...
// 指定キャスト・指定月の最新ステータス（未送信/送信済み・未確認/確認済み）
function getPayrollReceiptStatus_(name, month) {
  const sh = getPayrollReceiptSheet_();
  const rows = sh.getDataRange().getValues();
  for (let i = rows.length - 1; i >= 1; i--) {
    if (String(rows[i][1]).trim() === name && String(rows[i][0]).trim() === month) {
      return String(rows[i][6]).trim() === '確認済み' ? '確認済み' : '送信済み・未確認';
    }
  }
  return '未送信';
}

function sendPayrollReceipt_(month, targetName, amount, method) {
  const name = normalizeName_(targetName);
  const toUserId = getUserIdByName_(name);
  if (!toUserId) return { ok: false, error: name + ' さんのLINE登録が見つかりません' };

  const monthNum = (month.split('/')[1] || '').replace(/^0/, '');
  const message = [
    '【' + monthNum + '月給与】', '',
    '支給額：' + amount.toLocaleString() + '円',
    '支給方法：' + method, '',
    '内容をご確認いただき、', '「受領しました」', 'と返信してください。'
  ].join('\n');

  logPayrollReceiptRequest_(month, name, amount, method); // 送信前に記録（失敗時も追跡できるよう優先）
  push_(toUserId, message);
  return { ok: true };
}
...
function approveCashCheck(dateKey, approverName) {
  try {
    ...
    push_(prop('GROUP_KUROFUKU'), ['✅ 【閉店チェック承認済み】' + dateKey, '承認者　' + approverName, '黒服の退勤が可能になりました'].join('\n'));
...
function getCustomerMasterCols_(values) {
  ...
  return {
    ...
    ng: idx('NG行為'), ngStaff: idx('NGスタッフ'), regDate: idx('登録日')
  };
}
...
function getCustomerList() {
  ...
  for (let r = cols.headerRow + 1; r < values.length; r++) {
    ...
    results.push({
      ...
      tantou: String(val(row, cols.tantou)), bottle: String(val(row, cols.bottle)),
