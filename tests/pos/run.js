...
const SUITES = ['01_calc', '02_front', '03_backend', '04_contract'];
...
SUITES.filter(s => !only || s.indexOf(only) >= 0).forEach(name => {
  let mod;
  try { mod = require('./suites/' + name); } catch (e) { if (e.code === 'MODULE_NOT_FOUND' && String(e.message).indexOf(name) >= 0) return; throw e; }
  mod(front, back, { loadFront, loadBackend });
});
