...
module.exports = function (front) {
  ...
  {
    /* ⚠️④キャストの「同伴 ×回数」（¥3,000）と、有料メニューの「同伴 ¥3,000」が二重に存在する。
       両方打つと同伴料が2回乗る。W○○（ウェルカムの0円品）を消したのと同じ形の穴。 */
    const dohan = I.filter(r => r[0] === '同伴');
    if (dohan.length) {
      t.known('同伴が有料メニューに無い（④の同伴回数と一本化）',
              '⛔メニューの「同伴 ' + F.bmYen(dohan[0][1]) + '」と④キャストの同伴回数で**二重計上できる**。'
              + '\n       W○○を消したのと同じ形。消すかどうかはボス判断。');
    } else t.ok(true, '同伴が有料メニューに無い（④の同伴回数と一本化）');
  }
